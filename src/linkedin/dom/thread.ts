...
async function parseVisibleMessages(rows: Locator, conversationId: string, selfProfileUrl?: string, selfId?: string): Promise<{ messages: RawMessage[]; warnings: string[] }> {
  ...
  for (let index = 0; index < await rows.count(); index += 1) {
    ...
    messages.push({ ...(id ? { id } : {}), ...(entityUrn ? { entityUrn } : {}), conversationId, ...(senderId ? { senderId } : {}), ...(senderName ? { senderName } : {}), ...(senderProfileUrl ? { senderProfileUrl } : {}), ...(sentAt ? { sentAt } : {}), ...(direction ? { direction } : {}), text, sourceOrder: index, sourceMetadata: { sourcePage: `dom-thread:${conversationId}`, ...(!direction ? { directionEvidence: 'unknown-dom' } : {}) } });
  }
